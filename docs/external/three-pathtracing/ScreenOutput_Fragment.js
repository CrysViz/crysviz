...
// - wrapped as an exported string (no bundler/fetch step in this app);
// - source coordinates mapped via uAccumResolution/uOutputResolution so the
//   internal path-tracing resolution can be a fraction of the canvas
//   ("RT resolution" control);
...
void main()
{
	...
	// LOCAL ADAPTATION (CrysViz): map output pixels to accumulation-buffer texels
	// (the internal path-tracing resolution may be a fraction of the canvas)
	vec2 glFragCoord_xy = floor(gl_FragCoord.xy * (uAccumResolution / uOutputResolution)) + vec2(0.5);

	// LOCAL ADAPTATION (CrysViz): optional denoiser bypass (raw averaged samples)
	if (!uUseDenoiser)
	{
