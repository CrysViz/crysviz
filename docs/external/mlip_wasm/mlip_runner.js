...
//
// ---------------------------------------------------------------------------
// Verified conventions (node smoke test, mlip.js@0.1.1 CPU build, PET-MAD-xs):
//   * positions/forces are flat row-major per atom: [x0,y0,z0, x1,y1,z1, ...].
//     (central finite-difference dE/dx on atom 1 matched result.forces[3] to ~5e-4).
...
//   * STRESS: mlip.js@0.1.1 does NOT emit stress for PET-MAD — predict() returns
//     only { energy, forces } ("stress" never appears in the CPU glue). The
//     result shape below still carries a stress field for API parity, but it is a
//     ZERO tensor, which makes the relaxer's cell-deformation step a no-op (cell
//     stays fixed; atoms still relax). If a future mlip build emits stress, the
//     stressFromMlipVoigt() branch below converts it; its sign vs the repo's
//     tension-positive convention was NOT empirically determinable here and must
//     be re-verified against -dE/dV before trusting cell relaxation.
// ---------------------------------------------------------------------------
...
export class MLIPRunner {
  ...
  constructor(options = {}) {
    ...
    this.model = null;
    this.modelInfo = null;
    // mlip.js@0.1.1 emits no stress for PET-MAD; compute() returns a zero tensor
    // for shape parity. UI code reads this flag to show pressure/stress as n/a
    // and to disable cell-relaxation controls (NEPWasmRunner has no such flag,
    // i.e. `runner.supportsStress !== false` marks a real-stress runner).
    this.supportsStress = false;
  }
  ...
  async init() {
    if (!this.module) {
      ...
      this.module = await createMlipcpp({ backend: this.backend, cpuWasmUrl, gpuWasmUrl });
    }
    return this;
  }
  ...
  async compute(structure) {
    ...
    try {
      const r = await this.model.predict(sys);
