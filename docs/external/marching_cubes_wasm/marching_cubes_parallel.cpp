...
public:
	...
	size_t vertex2index(size_t x, size_t y, size_t z) {
		return z * z_step + y * y_step + x * x_step;
	}
	void index2vertex(size_t index, size_t& x, size_t& y, size_t& z) {
		// assumes order of vertices in field is x fastest, then y, then z (i.e. zyx order), and that x_step, y_step, z_step are set accordingly.
		z = (size_t)(index / z_step);
		y = (size_t)((index % z_step) / y_step);
		x = (size_t)((index % y_step) / x_step);
	}
	...
	void _add_edge_vertex(const float isoval,
			const unsigned char edge_ind,
			const size_t v1_ind, const size_t v2_ind,
	...
	void _countIsoCubes(const float isovalue, const size_t start_ind, const size_t end_ind, const unsigned int thread_ind) {
		...
		index2vertex(start_ind, x_start, y_start, z_start);
		size_t x_end, y_end, z_end;
		index2vertex(end_ind, x_end, y_end, z_end);
		...
		for (const auto& [vertex_index, edge_flags, cube_index] : this->pt_cube_cache[thread_ind]) {
			...
			for (int i = 0; i < 12; i++) {
				if (edge_flags & (1 << i)) { // iterate through bit flags for each edge, and if set, compute vertex position and normal for that edge
					const size_t v1_ind = vertex2index(x + cube_vertex_pos[edge2vertex[i][0]][0],
													   y + cube_vertex_pos[edge2vertex[i][0]][1],
													   z + cube_vertex_pos[edge2vertex[i][0]][2]);
					const size_t v2_ind = vertex2index(x + cube_vertex_pos[edge2vertex[i][1]][0],
													   y + cube_vertex_pos[edge2vertex[i][1]][1],
													   z + cube_vertex_pos[edge2vertex[i][1]][2]);
					...
					const float pos1[3] = {
						(x + (int)cube_vertex_pos[edge2vertex[i][0]][0])*dx,
						(y + (int)cube_vertex_pos[edge2vertex[i][0]][1])*dy,
						(z + (int)cube_vertex_pos[edge2vertex[i][0]][2])*dz
					};
