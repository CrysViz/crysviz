...
function createColorBar(container, colormap, minValue = "Min", maxValue = "Max") {
  const colorBarWrapper = createElement("div", {}, {
    display: "flex",
  ...
  const colorBarContainer = createElement("div", { class: "colorbar-container" }, {
    width: "120px",
    height: "40px",
    borderRadius: "3px",
    overflow: "hidden"
  ...
  const minLabel = createElement("div", {}, { fontSize: "12px", color: "rgba(255,255,255,0.9)" }, `${minValue} Å`);
  const maxLabel = createElement("div", {}, { fontSize: "12px", color: "rgba(255,255,255,0.9)" }, `${maxValue} Å`);
  ...
  return {
    update: (newColormap) => { colormap = newColormap; renderColorBar(); },
    setLabels: (newMin, newMax) => { minLabel.textContent = newMin; maxLabel.textContent = newMax; },
    remove: () => colorBarWrapper.remove(),
  };
}
...
export function bondLengthToColor(bondLength,minVal=1.1,maxVal=4.0) {
  let colors;
  switch (general.bondsColorMap) {
    case "batlow":
      colors = getBatlowColors();
