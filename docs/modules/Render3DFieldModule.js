...
function updateIsosurface(iso) {
  if (!groups.activeField || !groups.isosurface) return;

  groups.isosurface.updateMesh(iso);
}
...
function clearField() {
  if (groups.isosurface) {
    app.scene.remove(groups.isosurface);
    groups.isosurface.clearMesh();
  }
}
...
export function setActiveField(field, absoluteIsoValue = null, color1 = 0x33aaff, color2 = 0xff3333) {
  ...
  if (absoluteIsoValue === null && field.useAbsoluteIsoValue === null) {
    field.useAbsoluteIsoValue = field.minValue < 0; // store in field for future reference
  }

  // determine iso if not provided
  initIsosurfaceMesh(field);

  groups.activeField = field;
}

export function toggleFieldVisibility(visible) {
  groups.isosurface.visible = visible;
...
export function updateField(iso = null) {
  ...
  if (!groups.isosurface) {
    console.warn("Isosurface not initialized yet");
    return;
  }
  ...
  if (iso === null || iso === undefined) {
    if (field.isoValue !== 0) {
      ...
      if (field.useAbsoluteIsoValue) {
        min = field.absMinValue;
        max = field.absMaxValue;
      }
      else {
        min = field.minValue;
        max = field.maxValue;
  ...
  //--------------------------------------------------------
  app.scene.add(groups.isosurface);
}
