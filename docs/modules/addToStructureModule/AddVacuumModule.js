...
//------------------------------------------------------------
// Function: addVacuumAndAtomsPanel
// Adds a floating panel with tabs for atoms and vacuum
//------------------------------------------------------------
export function addVacuumPanel(buttonId = 'addButton') {
  ...
  button.addEventListener('click', () => {
    ...
    panel.innerHTML = `
      <div class="tabs" style="display: flex; margin-bottom: 10px;">
        <button class="tab-button active" data-tab="atoms" style="flex: 1; padding: 5px; cursor: pointer; background: #444; border: 1px solid #666; color: white;">Add Atoms</button>
        <button class="tab-button" data-tab="vacuum" style="flex: 1; padding: 5px; cursor: pointer; background: #444; border: 1px solid #666; color: white;">Add Vacuum</button>
      </div>

      <div id="atomsTab" class="tab-content active" style="display: block;">
        <div style="margin-bottom: 10px;">
          ...
          <div style="text-align: center; margin-top: 10px;">
            <button id="addNewRow" style="width: 90%; padding: 8px; background: #4CAF50; border: none; color: white; cursor: pointer; border-radius: 4px;">
              + Add New Atom
            </button>
          </div>
        </div>
...
O 1.5 1.5 1.5 #00FF00" style="flex: 1; height: 80px; background: #333; border: 1px solid #555; color: white; padding: 5px; resize: vertical; margin-right: 10px;"></textarea>
          <div style="display: flex; flex-direction: column;">
            <button id="selectElementForBulk" class="btn-mini highlight" style="padding: 5px 10px; border: none; color: white; cursor: pointer; margin-bottom: 5px; border-radius: 6px">Select Element</button>
            <button id="applyBulk" class="btn-mini highlight" style="padding: 5px 10px; border: none; color: white; cursor: pointer;border-radius: 6px">Apply Bulk</button>
          </div>
        </div>

        <div style="margin-top: 15px; text-align: right;">
          <button id="addToStructure" class="btn-mini highlight" style="margin-right: 10px; padding: 5px 10px; border: none; color: white; cursor: pointer;">Add to Structure</button>
          <button id="makeNewStructure" class="btn-mini highlight" style="padding: 5px 10px; border: none; color: white; cursor: pointer;">Make New Structure</button>
        </div>
      </div>

      <div id="vacuumTab" class="tab-content" style="display: none;">
        <div style="display: flex; align-items: center; margin-bottom: 10px;">
          <label style="margin-right: 10px;">X (Å):</label>
          <input type="number" id="vacX" value="0" step="1." style="width: 80px; background: #333; border: 1px solid #555; color: white; padding: 3px; margin-right: 10px;">

          <label style="margin-right: 10px; margin-left: 15px;">Y (Å):</label>
          <input type="number" id="vacY" value="0" step="1." style="width: 80px; background: #333; border: 1px solid #555; color: white; padding: 3px; margin-right: 10px;">

          <label style="margin-right: 10px; margin-left: 15px;">Z (Å):</label>
          <input type="number" id="vacZ" value="0" step="1." style="width: 80px; background: #333; border: 1px solid #555; color: white; padding: 3px;">

          <button id="applyVacuum" class="btn-mini highlight" style="margin-left: 15px; padding: 5px 10px; background: #4CAF50; border: none; color: white; cursor: pointer;">Apply Vacuum</button>
        </div>
      </div>
    ...
    // Vacuum tab logic
    document.getElementById('applyVacuum').addEventListener('click', () => {
    ...
    function addRowToTable(element = '', x = 0, y = 0, z = 0, color = '#000000') {
      const tbody = panel.querySelector('#atomsTable tbody');
    ...
    document.getElementById('selectElementForBulk').addEventListener('click', () => {
      currentBulkElementInput = document.getElementById('bulkInput');
      openPeriodicTable((element) => {
        if (currentBulkElementInput) {
          // Insert element at cursor position or append to existing text
    ...
    document.getElementById('applyBulk').addEventListener('click', () => {
      ...
      lines.forEach(line => {
        ...
        if (parts.length >= 4) {
          ...
          const color = parts[4] || '#000000'; // Default to black if no color specified

          // Add a new row to the table with these values
    ...
    document.getElementById('addToStructure').addEventListener('click', () => {
      const tbody = panel.querySelector('#atomsTable tbody');
      const rows = tbody.querySelectorAll('tr');
      ...
      atomsToAdd.length = 0; // Clear the array
    });

    // Make new structure button
    document.getElementById('makeNewStructure').addEventListener('click', () => {
      const tbody = panel.querySelector('#atomsTable tbody');
      const rows = tbody.querySelectorAll('tr');
