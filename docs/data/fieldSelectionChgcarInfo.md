...
- **Magnetization along σ₁**, **σ₂**, **σ₃** – a noncollinear run writes three magnetization blocks
  instead of one, one per Pauli matrix in the spin-quantization frame (the frame VASP's `SAXIS`
  defines), listed in file order.
