...
//   (f) persistence smoke — the 8 rtGround* keys survive at state v2.x.
'use strict';
...
(async () => {
  ...
  // --- (f) persistence smoke: the 8 rtGround* keys at state v2.x ------------------
  ...
  H.check('captureState carries all 8 rtGround* keys at v2.x',
    persist.version?.startsWith('2') && persist.present.length === 8,
