// Per-atom transparency draw order: a semi-transparent atom in front of an
// opaque atom must stay in front (blend over it), not vanish behind it.
// Regression test for the InstancedMesh depthWrite=false ordering bug: atoms
// render in instance-index order, so with depth writes off a later-indexed
// opaque atom overpainted an earlier transparent one. Fixed by the two-pass
// opaque/transparent split in AtomsFracUpdateModule (uAlphaPass + overlay).
//
...
(async () => {
  ...
  // --- Baseline: both opaque -> front red sphere fully covers the spot -----------
  ...
  // --- Transparent front atom must BLEND over the opaque back atom ---------------
  // Old bug: the later-indexed blue instance drew after the red one with depth
  // writes off and fully overpainted it -> pure blue disk (red collapses).
  await setOpacity(src.front, 0.5);
  await page.waitForTimeout(500);
  const s2 = await shoot('transparencyorder-front-transparent');
  H.check('transparent front atom lets the back atom show through (blue rises)',
    s2.b > s1.b + 30, JSON.stringify({ s1, s2 }));
  H.check('transparent front atom stays in front (red persists in the blend)',
  ...
  const state = await page.evaluate(async () => {
    ...
    return {
      mainTransparent: groups.atomsMesh.material.transparent,
      mainDepthWrite: groups.atomsMesh.material.depthWrite,
      overlayVisible: !!overlay?.visible,
    };
  });
  H.check('split passes: main material keeps depth writes, overlay pass visible',
    !state.mainTransparent && state.mainDepthWrite && state.overlayVisible,
    JSON.stringify(state));

  // --- Opaque front over transparent back: front must fully cover again ----------
  // Old bug: the later-indexed transparent back atom blended OVER the front one.
  await setOpacity(src.front, 1.0);
  await setOpacity(src.back, 0.5);
  await page.waitForTimeout(500);
  const s3 = await shoot('transparencyorder-back-transparent');
  ...
  const off = await page.evaluate(async () => {
    const { groups } = await import('./state/store.js');
    return {
      overlayVisible: !!groups.atomsMesh.userData.transparentOverlay?.visible,
      mainTransparent: groups.atomsMesh.material.transparent,
    };
  });
  H.check('overlay pass hides again when no atom is transparent',
    !off.overlayVisible && !off.mainTransparent, JSON.stringify(off));
