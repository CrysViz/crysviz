...
//       at N=512 the two images are near-identical;
//   (2) VARIANCE WIN: at equal low N (32), LDS-on is no worse than LDS-off in
//       mean-absolute-error against a near-converged (512-sample) reference —
...
(async () => {
  ...
  // (1) Identity: LDS on vs off at N=512 must converge to the same image.
  // N=128 was too few: two unbiased 128-sample renders still differ by MC
  // noise in ~1.1% of pixels (a 979x900 canvas; one 128-sample render is
  // already 0.8% away from its own 512-sample result), so the <1% gate failed
  // on noise alone. At 512 the on/off gap drops to ~0.1% changed, mean |ΔRGB|
  // 0.23, with a global signed bias of ~0.02/255 — convergence, not bias.
  const on512 = decodeDataUrl(await renderTo(page, 512, true));
  const off512 = decodeDataUrl(await renderTo(page, 512, false));
  const idMae = maePNG(on512, off512);
  const idChanged = changedFrac(on512, off512, 40);
  ...
  // (2) Variance win: MAE at N=32 against the LDS-on@512 reference; LDS-on must
  // be no worse than LDS-off (in practice clearly better — ratio logged).
  const ref = on512; // near-converged reference
  ...
  // Deterministic per machine (against the old 128-sample reference: maeOn=0.3733,
  // maeOff=0.4392, ratio 1.177; against the 512-sample one: maeOn=0.9041,
  // maeOff=1.4696, ratio 1.626 — bit-identical each run), so gate the win at
  // 0.95*maeOff with comfortable headroom.
